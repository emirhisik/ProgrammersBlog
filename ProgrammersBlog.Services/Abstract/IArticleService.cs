...
namespace ProgrammersBlog.Services.Abstract
{
    public interface IArticleService
    {
        ...
        Task<IDataResult<ArticleListDto>> GetAllByViewsCountAsync(bool isAscending, int? takeSize);
        ...
        Task<IDataResult<ArticleListDto>> GetAllByUserIdOnFilter(int userId, FilterBy filterBy, OrderBy orderBy, bool isAscending, int takeSize, int categoryId, DateTime startAt, DateTime endAt, int minViewsCount, int maxViewsCount, int minCommentCount, int maxCommentCount);
        Task<IDataResult<ArticleListDto>> SearchAsync(string keyword, int currentPage = 1, int pageSize = 5, bool isAscending = false);
        Task<IResult> IncreaseViewsCountAsync(int articleId);
